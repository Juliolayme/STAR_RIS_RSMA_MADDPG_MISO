...
$\mathbf{G}$ & Ma trận kênh từ BS đến STAR-RIS, $\mathbf{G} \in \mathbb{C}^{N \times M}$ \\
$\mathbf{h}_{r,k}^X$ & Vector kênh từ STAR-RIS đến UE thứ $k$, vùng $X \in \{R, T\}$ \\
$\mathbf{h}_{\mathrm{eff},k}$ & Vector kênh hiệu dụng MISO, được lưu sao cho $\mathbf{h}_{\mathrm{eff},k}^{H}\mathbf{w}=\mathbf{h}_{d,k}^{H}\mathbf{w}+\mathbf{g}_{k}^{H}\boldsymbol{\Phi}_{X}\mathbf{G}\mathbf{w}$ \\
