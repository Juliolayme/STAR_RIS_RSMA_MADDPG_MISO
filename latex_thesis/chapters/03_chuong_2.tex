...
Hệ thống được xét bao gồm: một trạm gốc (BS) với $M=4$ anten (cấu hình
MISO), một STAR-RIS với $N=32$ phần tử thụ động hoạt động ở chế độ chia
...
\begin{figure}[t]
  ...
  \caption{Cấu hình hệ thống STAR-RIS hỗ trợ RSMA: BS nhiều anten truyền tới
...
\subsection{Kênh hiệu dụng MISO}

Với $\mathbf{h}_{d,k}\in\mathbb{C}^{M}$, $\mathbf{G}\in
\mathbb{C}^{N\times M}$ và $\mathbf{g}_{k}^{X}\in\mathbb{C}^{N}$,
vector kênh hiệu dụng được lưu theo quy ước
\begin{equation}
  \mathbf{h}_{\mathrm{eff},k}
  = \mathbf{h}_{d,k}+\mathbf{G}^{H}\boldsymbol{\Phi}_{X}^{H}
    \mathbf{g}_{k}^{X}, \qquad X\in\{R,T\},
  \label{eq:heff}
\end{equation}
sao cho với mọi vector tiền mã hóa $\mathbf{w}\in\mathbb{C}^{M}$,
\begin{equation}
  \mathbf{h}_{\mathrm{eff},k}^{H}\mathbf{w}
  = \mathbf{h}_{d,k}^{H}\mathbf{w}
  + (\mathbf{g}_{k}^{X})^{H}\boldsymbol{\Phi}_{X}\mathbf{G}\mathbf{w}.
  \label{eq:heff_scalar}
\end{equation}
Quy ước này được kiểm chứng bằng oracle tín hiệu thu độc lập trong bộ kiểm
thử. Nghiệm căn pha giải tích sử dụng trực tiếp
$\mathbf{h}_{d,k}^{H}\mathbf{q}$ và
$g_{k,n}^{*}(\mathbf{G}_{n,:}\mathbf{q})$, tránh sai dấu liên hợp khi
đường trực tiếp có pha khác không.
...
trong đó trạng thái tập trung 681 chiều được dùng cho Critic, trong khi ba quan sát cục bộ được phân thành ba phần tương
ứng quan sát cục bộ của từng tác tử, không gian hành động 140 chiều bao
...
% LUU Y: hinh h2_5/h2_6 can ve lai theo so chieu MISO moi (681/821) sau refactor.
\begin{figure}[t]
  \centering
  \includegraphics[width=0.96\linewidth]{figures/h2_5_mdp_anh_xa.png}
  \caption{Ánh xạ bài toán P0 sang MDP: trạng thái tập trung 681 chiều
  (mỗi đặc trưng vật lý xuất hiện đúng một lần), không gian hành động 140 chiều,
...
Critic tập trung nhận trạng thái toàn cục $s_t\in\mathbb{R}^{681}$, trong đó mỗi đặc trưng vật lý chỉ xuất hiện một lần. Ba Actor vẫn nhận các quan sát cục bộ có kích thước lần lượt $73$, $577$ và $401$ (cấu hình chuẩn $M=4$, $K=4$, $K_R=3$, $N=32$).
Mỗi quan sát cục bộ bắt đầu bằng khối chung 41 chiều: phần thực và phần
ảo của kênh hiệu dụng $\mathbf{h}_{\text{eff},k}$ (32), vector trọng số
công suất ở bước trước (5) và tỷ lệ tách common-rate ở bước trước (4).
Biến phần thưởng bước trước không còn nằm trong quan sát để tránh phụ
thuộc vào hệ số đối ngẫu $\lambda$ cũ khi replay reward được tái tính.
Cụ thể:

\textbf{Tác tử BS (Agent 0, $o_0 \in \R^{73}$):} khối chung (41) cùng
phần thực/ảo của các kênh trực tiếp $\mathbf{h}_{d,k}$ cho cả 4 UE (32).

\textbf{Tác tử STAR-RIS Reflection (Agent 1, $o_1 \in \R^{577}$):} khối
chung (41); phần thực/ảo của $\mathbf{h}_{d,k}$ vùng R (24); của $\mathbf{G}$ (256);
của $\mathbf{g}_{k}^R$ với $k \in \{1,2,3\}$ (192); và trạng thái
STAR-RIS đang áp dụng: biên độ $\beta_n^R$ (32) cùng pha phản xạ hiện
hành $\phi_n^R$ (32).

\textbf{Tác tử STAR-RIS Transmission (Agent 2, $o_2 \in \R^{401}$):} khối
chung (41); phần thực/ảo của $\mathbf{h}_{d,4}$ (8); của $\mathbf{G}$ (256); của
$\mathbf{g}_{4}^T$ (64); và 32 pha truyền qua hiện hành $\phi_n^T$.
...
Tổng số chiều hành động là $44 + 64 + 32 = 140$:

\textbf{Tác tử BS (44 chiều):} 40 đại lượng phần thực/ảo của beamformer
phức cho common stream và 4 private streams, được chiếu chung lên quả cầu
công suất BS; 4 đại lượng còn lại tạo tỷ lệ tách $\{c_1,\ldots,c_K\}$ bằng
inverse-tanh logits rồi softmax để đảm bảo $\sum c_k = 1$ và vẫn tiếp cận
tốt biên simplex.
...
\begin{figure}[t]
  ...
  trung với đầu vào $681 + 140 = 821$ chiều.}
  \label{fig:h2_6}
\end{figure}
...
trạng thái tập trung 681 chiều, không gian hành động 140 chiều và hàm phần thưởng
