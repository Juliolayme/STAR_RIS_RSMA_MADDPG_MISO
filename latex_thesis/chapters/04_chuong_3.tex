...
nhất 105 chiều, hoặc cách triển khai $N$ tác tử (mỗi phần tử RIS một tác
...
luyện hơn so với một tác tử 105 chiều. Đồng thời, số tác tử vừa đủ
...
$Q_i: \R^{550} \times \R^{105} \to \R$ nhận vào toàn bộ quan sát ghép
...
\label{eq:critic_nn}
\end{equation}
Kích thước mỗi Critic: $655 \to 256 \to 256 \to 1$, không có activation
...
% -----------------------------------------------------------------------------
\section{Tham số hóa pha residual}
\label{sec:residual_phase}

\subsection{Động lực và ý tưởng}

Pha STAR-RIS $\theta_n^X \in [0, 2\pi)$ là biến tuần hoàn, không gian
khám phá rộng ($N=32$ pha, mỗi pha $2\pi$ rad). Nếu tác tử phải học
trực tiếp các giá trị pha tuyệt đối từ $\theta_n^X = 0$, tốc độ
hội tụ rất chậm vì xác suất ngẫu nhiên trùng vào vùng pha hợp lý ban đầu
là $O(1/(2\pi)^N) \approx 10^{-26}$ với $N=32$, gần như không xảy ra.

Để giải quyết, Luận văn đề xuất tham số hóa pha residual: thay vì
học pha tuyệt đối, tác tử học phần lệch (residual) so với một
tiên nghiệm phân tích bậc một (analytical prior). Tiên nghiệm này
là nghiệm xấp xỉ thu được từ điều kiện căn pha đồng pha cho người dùng
yếu nhất, có dạng giải tích.

\subsection{Nghiệm phân tích bậc một (Analytical Prior)}

Xét bài toán phụ: cho phép biên độ $\beta_n^X = 1/\sqrt{2}$ cố định, tìm
pha $\{\theta_n^X\}$ để cực đại $|h_{\text{eff},k_w^X}|$, với $k_w^X$ là
người dùng yếu nhất trong vùng $X$ (xác định bởi
$\arg\min_k \|\mathbf{h}_{r,k}^X\|^2$). Bài toán này có nghiệm dạng đóng
\cite{wu2020smart}, \cite{mu2022simultaneously}:
\begin{equation}
\theta_n^{\text{prior},X} \;=\; \arg(h_{d,k_w^X})
                              \;-\; \arg\!\big[(\mathbf{h}_{r,k_w^X}^X)_n^*\,(\mathbf{G})_n\big],
\label{eq:phase_prior}
\end{equation}
giúp các thành phần đường trực tiếp và đường gián tiếp cộng đồng pha tại
người dùng yếu nhất, tối đa hóa $|h_{\text{eff},k_w^X}|$.

\subsection{Công thức residual}

Tác tử pha học một phần dư $\Delta\phi_n^X \in [-1, 1]$ (đầu ra
tanh của mạng Actor) và pha thực được tạo theo công thức:
\begin{equation}
\boxed{
...
\end{equation}
Hệ số $\pi/4$ giới hạn phần dư trong $[-\pi/4, +\pi/4]$, tức $\pm 45^\circ$
quanh nghiệm phân tích. Không gian khám phá hữu hiệu của tác tử do đó
chỉ còn $(\pi/2)^N$ thay vì $(2\pi)^N$, giảm $4^N \approx 1{,}8 \times
10^{19}$ lần với $N=32$. Đồng thời, ngay từ bước đầu tiên (khi tác tử
chưa học gì), pha $\theta_n^X = \theta_n^{\text{prior},X}$ đã là một
lời giải hợp lý phần nào, cung cấp phần thưởng khởi đầu khác không cho
học. Biên độ residual $\pm\pi/4$ được lựa chọn qua khảo sát thực
nghiệm: biên rộng hơn ($\pm\pi/2$) khiến chính sách lệch xa nghiệm
phân tích và làm giảm sum-rate, trong khi biên hẹp hơn hạn chế khả
năng hiệu chỉnh đa người dùng của tác tử.
...
Với 3 tác tử và Critic kích thước $655 \to 256 \to 256 \to 1$:
\begin{equation}
\mathcal{C}_{\text{train,step}} = O\!\big(3 \times (256^2 + 256 \times 655)\big)
                                = O(10^5)\ \text{ops}.
\end{equation}
...
xuất: (i) tham số hóa pha residual giúp giảm không gian khám phá pha
